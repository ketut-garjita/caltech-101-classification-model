...
#### Option 1: AWS Lambda
   ...
   - Pre-requisites
     ...
     - Allow network access to AWS ECR endpoints. If using proxy/firewall ensure necessary ports are open. 
...
- Visualizations
  ...
  - True vs Prediction
    ...
    This is output of:
    - **./curl.sh** (on server)
    - **python curl_lambda.py** (Lambda, output file save on S3)
...
- Tools: TensorFlow, TensorFlow_Datasets, Pandas, Numpy, Matplotlib, Flask, Docker, AWS (Boto3, S3, IAM, ECR, Lambda, API Gateway, EC2).
